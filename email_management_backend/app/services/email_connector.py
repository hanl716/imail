...
def connect_imap(host: str, port: int, email_user: str, password: str, use_ssl: bool = True) -> imaplib.IMAP4:
    ...
    try:
        ...
        if status != 'OK':
            raise EmailLoginError(f"IMAP login failed for {email_user}: {response}")
        print(f"Successfully connected to IMAP server: {host} for user {email_user}")
        return imap_conn
...
def disconnect_imap(imap_conn: imaplib.IMAP4):
    ...
    try:
        ...
        # Log error but don't raise, as we are disconnecting anyway
...
def connect_smtp(
    ...
    try:
        ...
        print(f"Successfully connected to SMTP server: {host} for user {email_user}")
        return smtp_conn
...
def disconnect_smtp(smtp_conn: smtplib.SMTP):
    ...
    try:
        ...
        # Log error but don't raise, as we are disconnecting anyway
        print(f"Error during SMTP quit: {e}")
