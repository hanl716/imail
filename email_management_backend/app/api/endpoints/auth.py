...
@limiter.limit("10/hour") # Apply rate limit: 10 requests per hour
def register_user(request: Request, user: schemas.user.UserCreate, db: Session = Depends(get_db)): # Added request
...
@limiter.limit("5/minute") # Apply rate limit: 5 requests per minute
